...
class LocalHead2HeadMaster(EvaluatorMasterBase):
    ...
    def evaluate(self, iter_nr):
        ...
        print(f"[EVAL] eval stack size = {self._t_prof.eval_stack_sizes}")

        for stack_size_idx, stack_size in enumerate(self._t_prof.eval_stack_sizes):
            print(f"[EVAL] {stack_size_idx}, {stack_size}")
            for e in self._eval_agents:
            ...
            if do_it:
                ...
                mode = self._eval_agents[self._REFERENCE_AGENT].get_mode();
                filename = "single" if mode == 'SINGLE' else "avrg_net"
                print('saving exploitability...')
                with open(f'/home/lanhou/Workspace/Deep-CFR/assets/exploitability/{filename}.txt', 'a') as fp:
                    fp.write(f"iter = {iter_nr}, agent mode = {mode}, stack size idx = {stack_size_idx}, exploitability = {mean} \n")                 
            else:
                print(f"mode = {mode}, do it = {do_it}")
    ...
    def _run_eval(self, stack_size):
        ...
        for seat_p0 in range(self._env_bldr.N_SEATS):
            ...
            for iteration_id in tqdm(range(self._args.n_hands), desc='eval'):
