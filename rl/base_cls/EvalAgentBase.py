...
class EvalAgentBase:
    ...
    def notify_of_processed_tuple_action(self, p_id_acted, action_he_did):
        print(f"[NOTIFY] {self._internal_env_wrapper.env.current_player.seat_id}")
        print(f"[NOTIFY] {p_id_acted}")
        assert self._internal_env_wrapper.env.current_player.seat_id == p_id_acted
    ...
    def store_to_disk(self, path, file_name):
        # print(f"saving json... folder = {path}, filename = {file_name}")
        # full_path = os.path.join(path, f"{file_name}.json")
        # with open(full_path, "w") as fp:
        #     json.dump(self.state_dict(), fp, indent=4)
